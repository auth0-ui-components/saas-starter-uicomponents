...
export async function selectFeaturesInteractively() {
  const options = [
    {
      name: "Full SaaStart Experience",
      value: FeatureOptions.FULL,
      description:
        "Configure both My Organization and My Account APIs for the complete SaaStart demo (organization management + user self-service MFA).",
    },
    {
...
export function getFeatureDescription(featureConfig) {
  if (featureConfig.enableMyOrg && featureConfig.enableMyAccount) {
    return "Full SaaStart Experience (My Organization + My Account)"
